...
def weighted_mse(y_true, y_pred):
    weights_tensor = tf.constant([weights[0], weights[1], weights[2]], dtype=tf.float32)  # Apply weights
    squared_error = K.square(y_true - y_pred)  # Standard MSE formula
    return K.mean(weights_tensor * squared_error, axis=-1)  # Apply weights
...
def main():
    ...
    ######## NEED TO IMPLEMENT A FIX FOR UNBALANCED DATASET ^
    sys.exit()
